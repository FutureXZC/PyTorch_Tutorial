...
pretrained_dict = torch.load(
    os.path.join(".", "Code", "2_model", "net_params.pkl"))
net.load_state_dict(pretrained_dict)

# 在 PyTorch_Tutorial/Code 目录下运行
# writer = SummaryWriter(log_dir=os.path.join("..", ".."
#                                             "Result", "visual_weights"))
# 在 PyTorch_Tutorial 目录下运行
...
for k, v in params.items():
    if 'conv' in k and 'weight' in k:
        ...
        for j in range(c_out):
            ...
            kernel_grid = vutils.make_grid(kernel_j,
                                           normalize=True,
                                           scale_each=True,
                                           nrow=c_int)  # 1*输入通道数, w, h
            writer.add_image(k + '_split_in_channel',
                             kernel_grid,
                             global_step=j)  # j 表示feature map数
